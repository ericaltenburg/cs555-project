...
def recent_births(birth_date, name, id_num, lineNum):
	...
	thirty_days_since_birth = date.today()-(datetime.date(parse(birth_date)))
	

	if thirty_days_since_birth <= timedelta(days=30):
		info_str = "Info US19: "+name+" ("+id_num+") was born within the past 30 days on "+birth_date+" on line"+lineNum+"."
		print(info_str)
		return info_str
	else:
		return 
...
def recent_deaths(death_date, name, id_num, lineNum):
	thirty_days_since_death = date.today()-(datetime.date(parse(death_date)))
	if thirty_days_since_death <= timedelta(days=30):
		info_str = "Info US20: "+name+" ("+id_num+") has died within the past 30 days on "+death_date+" on line"+lineNum+"."
		print(info_str)
		return info_str
	else:
		return
